...
class DataSamplerModule:
    ...
    def data_sampler(self):
        ...
        if self.plot_bool:
            ...
            plt.close()
            self.corner_plot(np.asarray(H[:, 0], H[:, 1], H[:, 2]), labels= [r'$\langle F \rangle$', r'$T_0$', r'$\gamma$'], fig_name=f'{dir_plot}/corner_plot_train_{self.X_train.shape[0]}_seed_{self.seed}_params_loaded.png')

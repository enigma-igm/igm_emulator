...
DataLoader = DataSamplerModule(redshift=5.4,small_bin_bool=True,n_f=3, n_t=7, n_g=3,seed=36,plot_bool=False)   #total_data = 112
...
if __name__ == '__main__':
    ...
    def save_best_param_objective(trial):
        ...
        dill.dump(err_nn, open(
            ...
            'wb'))
        plot_covar_matrix(covar_nn / np.sqrt(np.outer(np.diag(covar_nn), np.diag(covar_nn))), name='covar_nn')
        plot_covar_matrix(np.sqrt(covar_nn)/np.sqrt(trainer.like_dict['covariance']+covar_nn)*100, name='sig_nn_frac_sig_tot')

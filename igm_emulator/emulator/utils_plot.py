...
def test_overplot(test_preds, Y_test, X_test, meanX,stdX,meanY,stdY, out_tag, var_tag,residual_plot = True):
    ...
    if residual_plot:
        ...
        for row in range(3):
            for col in range(3):
                axs2 = axes[row, col].inset_axes([0, 0.2, 1, 0.8], sharex=axes[row, col])  # Top subplot shares x-axis with parent
                new_ax = axes[row, col].inset_axes([0, 0, 1, 0.2], sharex=axes[row, col])

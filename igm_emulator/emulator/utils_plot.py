...
def test_overplot(test_preds, Y_test, X_test, meanX,stdX,meanY,stdY, out_tag, var_tag,residual_plot = True):
    ...
    for row in range(3):
        for col in range(3):
            ...
            yticks = [tick for tick in yticks if tick > 0] # remove negative and zero ticks
...
def plot_error_distribution(new_delta,out_tag, var_tag):
    ...
    ax.set_ylabel(r'$\left | \frac{\boldsymbol{\xi} - \boldsymbol{\xi}_\text{NN}}{\boldsymbol{\xi}} \right | \times 100$ [%]', fontsize=16)
